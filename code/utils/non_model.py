...
def cal_ssim(img1, img2, cuda_use=None):
    img1 = Variable(torch.from_numpy(deepcopy(img1).astype('float32'))).unsqueeze(0).unsqueeze(0)
    img2 = Variable(torch.from_numpy(deepcopy(img2).astype('float32'))).unsqueeze(0).unsqueeze(0)
